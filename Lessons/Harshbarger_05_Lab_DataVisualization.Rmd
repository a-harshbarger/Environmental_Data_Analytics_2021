...
SurfaceTemp <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), 
...
TempDepth <- ggplot(PeterPaul.chem.nutrients, 
...
# direction = 1 starts at purple end of spectrum
# direction = -1 starts at yellow end of spectrum (this makes more sense - deeper is darker)
...
AQIlat <- ggplot(EPAair, aes(x=PM2.5, y=Ozone, color=SITE_LATITUDE)) + 
...
PM2.5_1 <- ggplot(EPAair, aes(x=Year, y=PM2.5)) + 
  geom_point(size = 0.5)
print(PM2.5_1)

EPAair$Year <- as.factor(EPAair$Year)
PM2.5_2 <- ggplot(EPAair, aes(x=Year, y=PM2.5, color=Year))+
  geom_boxplot()
print(PM2.5_2)

PM2.5_3 <- ggplot(EPAair, aes(x=PM2.5, fill=Year))+
