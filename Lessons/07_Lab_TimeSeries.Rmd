...
BRA_plot <- ggplot(data = BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_point() + geom_smooth()
print(BRA_plot)

BRA_plot2 <- ggplot(data = BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_point() + geom_smooth()
print(BRA_plot2)
...
BRA_rain_ts <- ts(BRA_complete$Rainfall, start = c(1901, 1), frequency = 12) #c(year, month), freq=12 for monthly data
...
BRA_rain_Decomposed <- stl(BRA_rain_ts, s.window = "periodic")
print(BRA_rain_Decomposed)
plot(BRA_rain_Decomposed)

BRA_temp_Decomposed <- stl(BRA_temp_ts, s.window = "periodic")
print(BRA_temp_Decomposed)
