...
#setwd("C:/Users/Anne/Documents/ENV872/Environmental_Data_Analytics_2021/Lessons/12_Shiny/PeterPaulApp_Partial")
...
ui <- fluidPage(theme = shinytheme("yeti"),
  # Choose a title
  titlePanel("Peter and Paul Lake Nutrient Levels"),
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput(inputId = "x",
                  ...
                  value = c(as.Date("1995-01-01"), as.Date("2010-12-31")))),
    ...
    # Define reactive formatting for filtering within columns
     filtered_nutrient_data <- reactive({
        nutrient_data %>%
         ...
         filter(depth_id == input$fill) %>%
         # Filter for lakename selected by user
         filter(lakename == input$shape) 
     })
    ...
    # Create a ggplot object for the type of plot you have defined in the UI  
       output$scatterplot <- renderPlot({
        ggplot(filtered_nutrient_data(),#dataset
          ...
          labs(x = "Date", y = expression("Nutrient Concentration" ~ (mu*g / L)), shape = "Lake", fill = "Depth ID") +
